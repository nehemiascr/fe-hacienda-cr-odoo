...
class ElectronicInvoiceCostaRicaHacienda(models.AbstractModel):
    ...
    def _consultar_documento2(self, object):
        _logger.info('preguntando por %s' % object)
        ...
        try:
            url = self._get_url(object.company_id) + '/' + clave
            _logger.info('preguntando con %s' % url)
            response = requests.get(url, data=json.dumps({'clave': clave}), timeout=5)
    ...
    @api.model
    def _consultahacienda2(self, max_documentos=4):  # cron
        ...
        if invoice != None:
            ...
            for indice, factura in enumerate(facturas):
                _logger.info('Consultando documento %s / %s ' % (indice + 1, len(facturas)))
                if not factura.xml_comprobante: pass
                if self._consultar_documento2(factura): self.env['eicr.tools']._enviar_email(factura)
                self.env.cr.commit()
                max_documentos -= 1
        ...
        _logger.info('Consulta Hacienda 2 - Finalizado Exitosamente')
